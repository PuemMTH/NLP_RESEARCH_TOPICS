...
**Why:** User wants to see commands run inside their tmux session rather than Claude executing them silently. Output is visible in real-time and session persists across disconnects.
...
- Still use plain `ssh tl '...'` for read-only queries (`cat`, `ls`, `squeue`, `tmux ls`) where output goes back to Claude
